...
def heatmap_single_point(args):
    ...
    heatmap_imgs, heatmaps, trajectory_imgs, point_features, point_img = visualize_heatmaps_for_point(point, args.start_frame_idx, args.end_frame_idx, args.FRAMES_PATH, vit_extractor ,video_features = my_features, normalize_spatially=False)
...
def heatmap_grid_points(args):
    ...
    for query_point_idx, query_point in enumerate(query_points):
        heatmap_imgs, heatmaps, trajectory_imgs, point_features, point_img = visualize_heatmaps_for_point(query_point, args.start_frame_idx, args.end_frame_idx, args.FRAMES_PATH, vit_extractor, video_features = my_features, normalize_spatially=True)
        frames_path = save_video_frames(heatmap_imgs, output_path)
...
def heatmap_benchmark_grid_points(args):
    ...
    # Load benchmark query points for all frames
    benchmark_query_points_dictionary = get_query_points_from_benchmark_config(args.benchmark_pickle_path, args.video_id, rescale_sizes=[frames.shape[-1], frames.shape[-2]])  # x, y
    ...
    for frame_idx, query_points_list in benchmark_query_points_dictionary.items():
        print(f"Processing Query Points of {frame_idx} with {len(query_points_list)} query points")

        benchmark_query_points_frame = torch.tensor(query_points_list).cuda()  # Convert to tensor

        trajectory_query = []
        occlusion_query = []

        for query_point_idx, query_point in tqdm(
            enumerate(benchmark_query_points_frame), total=len(benchmark_query_points_frame), desc=f"Processing Frame {frame_idx}"
        ):
            if args.save_video:
                heatmap_imgs, heatmaps, trajectory_imgs, point_features, point_img, trajectory, occlusion = visualize_heatmaps_for_point(
                    query_point, args.start_frame_idx, args.end_frame_idx, args.FRAMES_PATH, vit_extractor=None, video_features=my_features, normalize_spatially=True
                )
                frames_to_video2(heatmap_imgs, output_filename=f"heatmap_frame_{frame_idx}_query_{query_point_idx}", fps=5, output_path=output_path)
            else:
                trajectory, occlusion = sift_optical_flow_get_trajectory_and_occlusion(
                    query_point, args.start_frame_idx, args.end_frame_idx, args.FRAMES_PATH, video_features=my_features, normalize_spatially=True
                )

            trajectory_query.append(trajectory)
            occlusion_query.append(occlusion)

        trajectory_query = np.array(trajectory_query)  # Shape: (num_points, num_frames, 2)
        occlusion_query = np.array(occlusion_query)  # Shape: (num_points, num_frames)

        print(f"Query Points Frame {frame_idx} - Trajectory Shape: {trajectory_query.shape}")
        print(f"Query Points Frame {frame_idx} - Occlusion Shape: {occlusion_query.shape}")

        # Ensure output directories exist
        os.makedirs(args.trajectory_output_path, exist_ok=True)
        os.makedirs(args.occlusion_output_path, exist_ok=True)

        # Save per-frame trajectory and occlusion
        ...
        np.save(trajectory_filename, trajectory_query)
        np.save(occlusion_filename, occlusion_query)

        print(f"Saved trajectory for frame {frame_idx} at: {trajectory_filename}")
        print(f"Saved occlusion for frame {frame_idx} at: {occlusion_filename}")

def sift_optical_flow_get_trajectory_and_occlusion(point, start_frame_idx, end_frame_idx, frames_path, vit_extractor=None, video_features=None, normalize_corr=True, normalize_spatially=True, occlusion_threshold=0.3):
    import cv2
    import numpy as np
    import torch.nn.functional as F

    frames = load_video(frames_path)["video"]
    frames_np = frames.permute(0, 2, 3, 1).cpu().numpy()  # Convert to (T, H, W, C)
    _t, c, h, w = frames.shape
    ...
    point_frame_idx = int(point[2])
    point_frame = frames[point_frame_idx].cuda()
    point_frame_features = video_features[point_frame_idx].to(device)

    # Extract DINO feature for the point
    samples = point[:2][None, None, None, ...].detach().clone().float()
    samples[:, :, :, 0] = samples[:, :, :, 0] / (w - 1)
    samples[:, :, :, 1] = samples[:, :, :, 1] / (h - 1)
    samples = samples * 2 - 1
    point_feature = F.grid_sample(point_frame_features[None, ...], samples, align_corners=True)[:, :, 0, 0]

    # Extract SIFT descriptor at query point
    x, y = int(point[0].item()), int(point[1].item())
    sift = cv2.SIFT_create()
    rgb_frame = (frames_np[point_frame_idx] * 255).astype(np.uint8)
    gray_frame = cv2.cvtColor(rgb_frame, cv2.COLOR_RGB2GRAY)
    kp_query = [cv2.KeyPoint(x, y, 1)]
    _, des_query = sift.compute(gray_frame, kp_query)
    if des_query is None:
        des_query = np.zeros((1, 128), dtype=np.float32)
    ...
    for t in range(start_frame_idx, end_frame_idx + 1):
        ...
        _, heatmap = compute_correspondence_heatmap(point_feature, frame_features, h_f, w_f, normalize_corr=normalize_corr, normalize_spatially=normalize_spatially)

        max_idx = heatmap.argmax()
        nearest_coord = unravel_index(max_idx, heatmap.shape)
        heatmap_value = heatmap[nearest_coord]

        if heatmap_value < occlusion_threshold:
            occlusion.append(True)
            nearest_coord = None
            trajectory.append((int(nearest_coord[1] * w / w_f), int(nearest_coord[0] * h / h_f)))
            continue
        else:
            occlusion.append(False)

        coarse_y = nearest_coord[0] * h / h_f
        coarse_x = nearest_coord[1] * w / w_f

        patch_size = 4
        x1 = int(max(0, coarse_x - patch_size))
        x2 = int(min(w, coarse_x + patch_size))
        y1 = int(max(0, coarse_y - patch_size))
        y2 = int(min(h, coarse_y + patch_size))

        rgb_patch = (frames_np[t][y1:y2, x1:x2] * 255).astype(np.uint8)
        gray_patch = cv2.cvtColor(rgb_patch, cv2.COLOR_RGB2GRAY)
        kp_target, des_target = sift.detectAndCompute(gray_patch, None)

        if des_target is None or len(kp_target) == 0:
            trajectory.append((int(coarse_x), int(coarse_y)))
            continue

        matches = bf.match(des_query, des_target)
        if matches:
            match = matches[0]
            pt = kp_target[match.trainIdx].pt
            refined_x = x1 + pt[0]
            refined_y = y1 + pt[1]
        else:
            refined_x, refined_y = coarse_x, coarse_y
        
        # Optical flow refinement (optional)
        if t < end_frame_idx:
            gray_curr = cv2.cvtColor((frames_np[t] * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY)
            gray_next = cv2.cvtColor((frames_np[t + 1] * 255).astype(np.uint8), cv2.COLOR_RGB2GRAY)

            prev_pts = np.array([[[refined_x, refined_y]]], dtype=np.float32)
            next_pts, status, _ = cv2.calcOpticalFlowPyrLK(gray_curr, gray_next, prev_pts, None, winSize=(15,15), maxLevel=2)

            if status[0][0] == 1:  # valid tracking
                refined_x, refined_y = next_pts[0][0]

        trajectory.append((int(refined_x), int(refined_y)))

    return trajectory, occlusion

def get_trajectory_and_occlusion(point, start_frame_idx, end_frame_idx, frames_path, vit_extractor = None, video_features=None, normalize_corr=True, normalize_spatially=True, occlusion_threshold=0.3):
    # compute the trajectory and occlusion for a single point
    frames = load_video(frames_path)["video"]
    _t, c, h, w = frames.shape
    ...
    for t in range(start_frame_idx, end_frame_idx+1):
        ...
        nearest_coord = unravel_index(heatmap.argmax(), heatmap.shape)  # h_f, w_f
        # Get the heatmap value at the nearest_coord
        heatmap_value = heatmap[nearest_coord]
        # If the heatmap value is below the occlusion threshold, set as occlusion
        if heatmap_value < occlusion_threshold:
...
def eff_heatmap_benchmark_grid_points(args):
    print("loading video features")
    my_features = torch.load(args.dino_embed_video_path)
    frames = load_video(args.FRAMES_PATH)["video"].cuda()
    ...
    for frame_idx, query_points_list in benchmark_query_points_dictionary.items():
        ...
        trajectory_query, occlusion_query = eff_get_trajectory_and_occlusion(
                    benchmark_query_points_frame, args.start_frame_idx, args.end_frame_idx, args.FRAMES_PATH, video_features=my_features, normalize_spatially=True
                )
...
def eff_get_trajectory_and_occlusion(points, start_frame_idx, end_frame_idx, frames_path, vit_extractor=None, video_features=None, normalize_corr=True, normalize_spatially=True, occlusion_threshold=0.3):
    frames = load_video(frames_path)["video"]
    _t, c, h, w = frames.shape

    #print("points", points)
    num_points = points.shape[0]
    point_frame_idxs = points[:, 2].long().cpu()  # Get frame indices for each point
    #print("point_frame_idxs", point_frame_idxs)

    # Extract feature maps for each initial point frame
    point_frames = frames[point_frame_idxs].cuda()
    point_frame_features = video_features[point_frame_idxs].cuda()  # Shape: (num_points, C, H_f, W_f)
    #print("point_frame_features", point_frame_features.shape)
    
    # Normalize the point coordinates for grid sampling
    samples = points[:, :2].detach().clone().float()
    samples[:, 0] = samples[:, 0] / (w - 1) * 2 - 1  # Normalize X
    samples[:, 1] = samples[:, 1] / (h - 1) * 2 - 1  # Normalize Y
    samples = samples.view(num_points, 1, 1, 2)  # Shape: (num_points, 1, 1, 2)

    # Extract point features using grid_sample
    point_features = torch.nn.functional.grid_sample(point_frame_features, samples, align_corners=True)  # (num_points, C, 1, 1)
    
    # Reshape to (num_points, C)
    point_features = point_features.view(num_points, -1)  # Flatten to (num_points, C)
    #print("point_features", point_features.shape)  # Now it will be (num_points, C)
    ...
    for t in tqdm(range(start_frame_idx, end_frame_idx + 1), desc="Processing frames"):
        ...
        _, heatmaps = eff_compute_correspondence_heatmap(point_features, frame_features, h_f, w_f, normalize_corr=normalize_corr, normalize_spatially=normalize_spatially)
        ...
        nearest_coords = torch.stack(nearest_coords, dim=1)  # (num_points, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dino_model", type=str, default="dinov2_vitbs4")
    ...
    for folder_name in sorted(os.listdir(args.dataset_root)):
        ...
        args.dino_embed_video_path = os.path.join(folder_path, "dino_small_embeddings/dino_embed_video.pt") # for dinov2 
        args.output_path = os.path.join(output_video_folder, "dinov2")
        args.trajectory_output_path = os.path.join(output_video_folder, "dinov2_sift_opt_grid_trajectory")
        args.occlusion_output_path = os.path.join(output_video_folder, "dinov2_sift__opt_grid_occlusion")
        ...
        if args.point == 'grid':
            ...
            heatmap_benchmark_grid_points(args)
        else:
