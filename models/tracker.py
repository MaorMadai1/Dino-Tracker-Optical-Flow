...
class Tracker(nn.Module):
    ...
    def save_weights(self, iter):
        torch.save(self.tracker_head.state_dict(), Path(self.ckpt_path) / f"tracker_head_small{iter}.pt")
        torch.save(self.delta_dino.state_dict(), Path(self.ckpt_path) / f"delta_dino_small{iter}.pt")
        
    def load_weights(self, iter):
        self.tracker_head = load_pre_trained_model(
            torch.load(os.path.join(self.ckpt_path, f"tracker_head_small{iter}.pt")),
            self.tracker_head
        )
        self.delta_dino = load_pre_trained_model(
            torch.load(os.path.join(self.ckpt_path, f"delta_dino_small{iter}.pt")),
            self.delta_dino
        )
