...
def get_last_ckpt_iter(folder_path):
    """
    Get the latest checkpoint iteration from files in the folder that contain the word 'small'.

    Args:
        folder_path (str): Path to the folder containing checkpoint files.

    Returns:
        int: The highest iteration number found, or -1 if no valid checkpoints exist.
    """
    files = os.listdir(folder_path)
    # Filter files that contain the word "small"
    file_paths = [
        os.path.join(folder_path, file)
        for file in files
        if os.path.isfile(os.path.join(folder_path, file)) and "small" in file
    ]
