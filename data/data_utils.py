...
def load_video(video_folder: str, resize=None, num_frames=None):
    ...
    for file in input_files:
        if resize is not None:
            video.append(transforms.ToTensor()(Image.open(str(file)).resize((resw, resh), Image.LANCZOS))) # LANCZOS is a high-quality downsampling filter for PIL
            #video.append(transforms.ToTensor()(Image.open(str(file)).resize((resw, resh))))
        else:
...
def frames_to_video(frames_path, output_filename, fps=30, output_path=None) -> None:
    """
    Converts a list of frames to an mp4 video. file located in the folder (NOTE: could cause me issues later).
    """
    images = sorted(list(Path(frames_path).glob("*.jpg")) + list(Path(frames_path).glob("*.png")))
    # remove files that contain any alphabet characters
    images = [image for image in images if not any(char.isalpha() for char in image.stem)]
    video_np = np.stack([iiov3.imread(image) for image in images])
    out_file_path = Path(frames_path) / f"{output_filename}_fps_{fps}.mp4" if output_path is None else output_path
    iiov3.imwrite(out_file_path, video_np, fps=fps, macro_block_size=None)

def frames_to_video2(frames, output_filename, fps=30, output_path=None) -> None:
    """
    Converts a list of frames (numpy arrays) to an mp4 video.
    
    Args:
        frames (list): List of frames as numpy arrays.
        output_filename (str): The name of the output video file.
        fps (int): Frames per second for the video.
        output_path (str or None): Path to save the output video. If None, it saves in the current directory.
    """
    # Ensure frames is a numpy array and stack into a single 4D array (T, H, W, C)
    video_np = np.stack(frames, axis=0)    
    # Set the output file path
    out_file_path = Path(output_path) / f"{output_filename}_fps_{fps}.mp4" if output_path else Path(output_filename + f"_fps_{fps}.mp4")   
    # Write the video
    iiov3.imwrite(out_file_path, video_np, fps=fps)
    print(f"Video saved at: {out_file_path}")
...
def get_grid_query_points(res_h_w, segm_mask=None, device="cuda", interval=None, query_frame=0):
    ...
    if segm_mask is not None:
        ...
        segm_mask = F.interpolate(segm_mask, size=(res_h_w[0], res_h_w[1]), mode="nearest")
        segm_mask = segm_mask.squeeze(0).squeeze(0)  # Convert back to 2D
